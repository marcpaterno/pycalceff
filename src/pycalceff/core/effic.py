...
def compute_hpd_interval_k_zero(
    ...
    """
    low = 0.0
    high = searchupper(low, 0, ntrials, conflevel)
    return low, high
...
def compute_hpd_interval_k_ntrials(
    ...
    """
    high = 1.0
    low = searchlower(high, ntrials, ntrials, conflevel)
    return low, high


def compute_hpd_interval_general(
    k: int, ntrials: int, conflevel: float
) -> tuple[float, float]:
    """
    Compute the highest posterior density (HPD) interval
    for 0 < k < ntrials.
    ...
    # Use brentq to find h such that the mass above h equals conflevel
    def mass_diff(h: float) -> float:
        def f(x: float) -> float:
            return posterior_density(x, k, ntrials) - h

        a = brentq(f, 0, mode, xtol=1e-14)
        b = brentq(f, mode, 1, xtol=1e-14)
        mass = beta_ab(a, b, k, ntrials)
        return mass - conflevel

    h = float(brentq(mass_diff, 0.0, posterior_density(mode, k, ntrials)))

    # Find the roots with the converged h
    def g(x: float) -> float:
        return posterior_density(x, k, ntrials) - h

    a = brentq(g, 0, mode, xtol=1e-14)
    b = brentq(g, mode, 1, xtol=1e-14)
    return a, b
...
def compute_hpd_interval(
    ...
    """
    if k == 0:
        return compute_hpd_interval_k_zero(ntrials, conflevel)
    elif k == ntrials:
        return compute_hpd_interval_k_ntrials(ntrials, conflevel)
    else:
        return compute_hpd_interval_general(k, ntrials, conflevel)
...
def searchupper(low: float, k: int, numtrials: int, c: float) -> float:
    """
    ...
    # Use scipy.optimize.bisect for root finding
    def func(x: float) -> float:
        return beta_ab(low, x, k, numtrials) - c

    try:
        return float(bisect(func, low, 1.0, xtol=1e-12))
    except ValueError:
        raise ValueError(
...
def searchlower(high: float, k: int, ntrials: int, c: float) -> float:
    """
    ...
    try:
        return float(bisect(func, 0.0, high, xtol=1e-12))
    except ValueError:
        raise ValueError(
...
def interval(low: float, k: int, N: int, conflevel: float) -> float:
    ...
    """
    high = searchupper(low, k, N, conflevel)
    return high - low
...
def effic(
    ...
    if k == 0:
        low, high = compute_hpd_interval(k, ntrials, conflevel)
    elif k == ntrials:
        low, high = compute_hpd_interval(k, ntrials, conflevel)
    else:
        low, high = compute_hpd_interval(k, ntrials, conflevel)
